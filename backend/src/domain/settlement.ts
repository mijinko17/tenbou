...
export function computeSettlement(
	...
	for (const p of players) {
		finalBalance.set(
			p.id,
			(subtotalG.get(p.id) ?? 0) + (advanceAdj.get(p.id) ?? 0),
		);
	}

	// Step 8: Greedy法で支払い最小化
	const mutableBalances = players.map((p) => ({
		playerId: p.id,
		balance: finalBalance.get(p.id) ?? 0,
	}));
	...
	const breakdown = players.map((p) => ({
		...
		finalBalance: finalBalance.get(p.id) ?? 0,
	}));
