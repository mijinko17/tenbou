...
- 各行の末尾に削除ボタン（ゴミ箱アイコン）を表示する
- ボタン押下で確認ダイアログを表示し、確認後に削除する
- 削除すると `game_rounds` レコードと紐づく `game_results` レコードをすべて削除する
- `round_no` はDB上は詰め直さないが、フロントエンドは配列インデックス（1始まり）で表示するため、ユーザには常に連番で見える

### チップの削除

- チップ行の末尾に削除ボタン（XIcon）を表示する
- ボタン押下で確認ダイアログを表示し、確認後に削除する
- 削除すると `chip_totals` の該当グループの全レコードを削除する
- 削除後はチップ行が非表示になる
...
- セッションがこのグループに属すること
- `game_results`（紐づく成績）を先に削除してから `game_rounds` を削除する
- 成功時: 204 No Content
- 存在しない roundId: 404
